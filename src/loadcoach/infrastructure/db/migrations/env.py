"""Alembic environment for LoadCoach's own migration history.
...
def run_migrations_online() -> None:
    ...
    database standards §2). Adding a constraint to an existing SQLite table is a table rebuild —
    alembic's batch mode copies, drops and renames — and dropping a table that other rows
    reference ``ON DELETE CASCADE`` deletes those rows. Migration 0009 adds a foreign key to
    ``routing_decisions``, whose ``routing_candidates`` children are the explainability promise
    itself, so the rebuild must not cascade. The pragma takes effect only outside a transaction,
    which is why it runs before ``begin_transaction`` and is restored after.
    """
